...
//过滤搜索的值
function check_sqlValue($values){
...
//*号显示某些内容
function xing_name($name){
	$count = mb_strlen($name,'utf-8');
	if($count <= 4){
		$half = ceil($count/2);
		echo mb_substr($name,0,$half,'utf-8').str_repeat('*',$half);
	} else {
		$half = floor(($count-2)/2);
		$half = $half > 1 ? $half : 2;
		$ceil = ceil($half/2);
	//	echo $name.'<br>';
	//	echo $half.'-'.$ceil.'<br>';
		echo mb_substr($name,0,$half,'utf-8').str_repeat('*',$half).mb_substr($name,-$ceil,$ceil,'utf-8');
	}
}
...
function strcut($string = '', $length = '') {
	...
	//判断金额
	function check_start_price($money){
		if(!preg_match('/^(d+|[1-9]).d{0,2}$/', $money))return true;
		return true;
	}
	...
	function sendMail($to, $title, $content) {
        ...
        $mail->IsSMTP(); // 启用SMTP
        $mail->Host=C('USER_CONFIG.email_stmp'); //smtp服务器的名称
        $mail->SMTPAuth = true; //启用smtp认证
        $mail->Username = C('USER_CONFIG.email_user'); //你的邮箱名
        $mail->Password = C('USER_CONFIG.email_password'); //邮箱密码
        $mail->From = C('USER_CONFIG.email_user'); //发件人地址（也就是你的邮箱地址）
        $mail->FromName = C('USER_CONFIG.email_sendname'); //发件人姓名
        $mail->AddAddress($to);
