...
//                    ->addRightButton('forbid', array('model' => $this->model))  //添加禁用/启用按钮
    ...
    public function add($cid) {
        if (IS_POST) {
            ...
            //排除不需要的字段数组 目前有：封面cover，标签abstract，简介tags zss
//           $del_name = array('abstract');
            if($cid == 1 || $cid == 5){
                $del_name = array('abstract', 'tags', 'file');
            } else {
               $del_name = array('abstract', 'tags', 'file','cover');                 
            }
            ...
            if ($field_sort) {
                ...
                foreach ($field_sort as $k1 => &$v1) {
                    ...
                    foreach ($v1 as $k2 => $v2) {
                    	...
                    	//将不需要的取消掉
                   	if(in_array($new_attribute_list[$v2]['name'], $del_name))unset($new_attribute_list[$v2]);
            ...
            $new_attribute_list[0]['options']['group1']['options'][6]['tip'] = '支持格式:(*.doc、*.xls、*.txt),仅支持一个文件,默认10M以内(可配置)';
	...
	$article_list = D('Article')->getNewList(3,10,'',$map);
//        var_dump($article_list);
        $page = new Page(D('Article')->where($map)->count(), C('ADMIN_PAGE_ROWS'));
		
       //顶部还原与删除按钮
       $top_del['href'] =  U('delete', array('status' => 'delete'));
       $top_huanyuan['href'] =  U('delete', array('status' => 'delete','type'=>'hy'));
        
        //行内还原与删除按钮
        $huanyuan['href'] = U('delete', array('ids' => '__data_id__','type'=>'hy'));
        $del['href'] = U('delete', array('ids' => '__data_id__'));
        
        //使用Builder快速建立列表页面。
        $builder = new \Common\Builder\ListBuilder();
        $builder->setMetaTitle('回收站') //设置页面标题
                ->addTopButton('delete', $top_del) //添加删除按钮
                ->addTopButton('restore', $top_huanyuan) //添加还原按钮
                ...
                ->setTableDataList($article_list) //数据列表
                ->setTableDataPage($page->show()) //数据列表分页
                ->addRightButton('restore', $huanyuan) //添加还原按钮
                ->addRightButton('delete', $del) //添加删除按钮
                ->display();
    }
    ...
    public function delete(){
    	...
    	if(empty($_REQUEST['ids']))$this->error('请先选择信息');
    	$map['id'] = array('in', $_REQUEST['ids']);
    	if($_REQUEST['type'] == 'hy'){
    		M('ArticleBase')->where($map)->save(array('status'=>1));
    	} else {
    		M('ArticleArticle')->where($map)->delete();
    		M('ArticleBase')->where($map)->delete();
    	}
    	$this->success('操作成功');
    }
}
