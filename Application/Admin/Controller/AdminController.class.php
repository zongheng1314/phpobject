...
class AdminController extends CommonController {
    ...
    protected function _initialize() {
    	...
    	//验证如果学校单位被删除。那么重新登录
    	if(!in_array(1, session('user_group')) && session('dep_id')){
	    	$dep = M('Department')->where('id = '.session('dep_id'))->find();
	    	if($dep == false){
		    	session('user_auth', null);
		        session('user_auth_sign', null);
		        $this->error('获取信息超时！请重新登录', U('login'));
	    	}
    	}
        // 权限检测
	    ...
	    if('Admin/Index/index' !== $current_url && 'Admin/Log/index' !== $current_url){
	    	D('Admin/Log')->add_log();	//写入日志
	    }
 ...
 */	
	public function get_area(){
		...
		if(in_array(1, session('user_group'))){
			...
			if($_POST['area_deep'] == 2){
				$city = session('city');
				$map['area_deep'] = $_POST['area_deep'];
				if($city[$_POST['id']] == 'all'){
					$map['area_parent_id'] = $_POST['id'];
				} else {
					$map['area_id'] = array('in', $city[$_POST['id']]);
					$map['area_parent_id'] = $_POST['id'];
				}
				$content = $model->where($map)->order('area_sort')->select();
				$this->ajaxReturn($content);
			}
			if($_POST['area_deep'] == 3){
				$qu = session('qu');
				$city = session('city');
				$map['area_deep'] = $_POST['area_deep'];
				$city_id = $model->field('area_parent_id')->where('area_id = '.$_POST['id'])->find();
				if($qu[$_POST['id']] == 'all' or $city[$city_id['area_parent_id']] == 'all'){
					$map['area_parent_id'] = $_POST['id'];
				} else {
					$map['area_id'] = array('in', $qu[$_POST['id']]);
					$map['area_parent_id'] = $_POST['id'];
				}
				$content = $model->where($map)->order('area_sort')->select();
				$this->ajaxReturn($content);
...
/**
 * 获取下级属性
 * @author  周书森  2015/12/30
 */	
	public function get_attrtype(){
		if($_POST['deep'] == 1){
			$content = M('Attribute')->where('type_id = '.$_POST['id'])->order('attr_sort desc,id desc')->select();
		} else {
			$content = M('AttributeValue')->where('attr_id = '.$_POST['id'])->order('attr_value_sort desc,id desc')->select();
		}
