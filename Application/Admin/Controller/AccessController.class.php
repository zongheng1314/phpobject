...
class AccessController extends AdminController {
    ...
    public function add(){
        if (IS_POST) {
            ...
            if ($data) {
            	$uid = M('AdminUser')->where('username = "'.$_POST['username'].'"')->field('id')->find();
            	if($uid == false)$this->error('该用户不存在');
            	$re = D('Access')->where('uid = "'.$uid['id'].'"')->find();
            	if($re)$this->error('该用户已存在用户组');
            	$data['uid'] = $uid['id'];
            ...
            $builder->setMetaTitle('新增配置')  //设置页面标题
                    ...
                    ->addFormItem('username', 'text', '用户名', '用户账号非昵称')
    ...
    public function edit($id){
        if (IS_POST) {
        	...
        	$data = D('Access')->where('auc_admin_access.id = '.$_GET['id'])
        			->join('auc_admin_user U on auc_admin_access.uid = U.id')
        			->field('auc_admin_access.*, U.username')
        			->find();
            // 使用FormBuilder快速建立表单页面。
            $builder = new \Common\Builder\FormBuilder();
