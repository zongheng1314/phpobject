...
class UploadModel extends Model {
     ...
     * zss 20160718 09:01
     * $field_id 文件ID
     * $field_format 文件类型（array）
     * 通过返回 文件路径。失败返回false
     */
    
    public function check_format($field_id , $field_format = array()){
    	$file_info = $this->find($field_id);
    	if(in_array($file_info['ext'], $field_format)){	//格式正确返回文件路径
    		return $file_info;
    	} else {	//不正确删掉无用文件
    		unlink($file_info['path']);
    		$this->where('id = '.$field_id)->delete();
    		return false;
    	}
