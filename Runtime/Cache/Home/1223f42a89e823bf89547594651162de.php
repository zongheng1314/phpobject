    ...
    <table class="main-table" cellpadding="0" cellspacing="0" border="0">
        <tr>
            <td width="300" class="inner-left">
                ...
                <ul class="news-class">
                    <?php if(is_array($park_child)): $i = 0; $__LIST__ = $park_child;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$park): $mod = ($i % 2 );++$i;?><!--<li <?php if($article_detail['cid'] == $park['id']): ?>class="on"<?php endif; ?>--> 
                    <li <?php if($_GET['pid'] == $park['id'] || $article_detail['cid'] == $park['id']): ?>class="on"<?php endif; ?>><a href="<?php echo U('Index/volist', array('pid' => $park['id']));?>"><?php echo strcut($park['title'],15);?></a></li>
                      <!--<li class="on"><a href="#"><?php echo ($park['title']); ?></a></li>--><?php endforeach; endif; else: echo "" ;endif; ?>
                </ul><?php endif; ?>
                <div class="left-t">最新文章</div>
                <ul class="news-hot">
                    <?php if(is_array($new_article_list)): $i = 0; $__LIST__ = array_slice($new_article_list,0,6,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$newarticle): $mod = ($i % 2 );++$i;?><li><a href="<?php echo U('Index/detail',array('id'=>$newarticle['id']));?>"><?php echo strcut($newarticle['title'],10);?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
                </ul>
            ...
            <td width="685" class="inner-right">
                <div class="position">
                    ...
                    <?php if(!empty($group_list["1"])): ?><a href="<?php echo U('Index/volist',array('pid' =>$group_list[0]['id']));?>"><?php echo ($group_list[0]['title']); ?></a> ><?php endif; ?>
                    <?php if(!empty($up_title)): ?><a href="<?php echo U('Index/volist',array('pid' =>$group_list[count($group_list)-1]['id']));?>"><?php echo ($up_title); ?></a> ><?php endif; ?>
                   
                    <a ><?php echo strcut($article_detail['title'],15);?></a>
                </div>
                <!--<?php echo U('Index/volist',array('pid' =>$group_list[0]['id']));?>-->
                <ul class="right-list">
                    
                    <!--内容模块-->
                        <div class='article_title'><?php echo ($article_detail['title']); ?></div>
                        <div class="update_time"><?php echo (date("Y-m-d H:i:s",$article_detail['update_time'])); ?></div>
                       
                        <div class="art_content"><?php echo ($article_detail['content']); ?></div>
                </ul>
